...
Uses parallel HTTPS POST streaming against an Ookla server.  Design mirrors
the download module: shared session, warm-up discard, IQM-based final speed,
...
class UploadTester:
    """
    Parallel upload speed tester.

    Each worker opens a long-running streaming POST request.  A sampler
    coroutine records throughput every ``SAMPLE_INTERVAL`` seconds,
    discarding the first ``WARMUP_SECONDS``.  The final speed is the IQM
    of the post-warmup samples.
    """

    def __init__(self, duration_seconds: float = 15.0) -> None:
    ...
    async def test(self, server: Server, connections: int = 4) -> UploadResult:
        ...
        async def _worker(session: aiohttp.ClientSession, cid: int) -> None:
            ...
            while not stop.is_set() and time.perf_counter() < end_time:
                try:
                    async with session.post(server.upload_url, data=_data_stream()) as resp:
                        await resp.read()
